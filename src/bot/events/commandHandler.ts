...
/**
 * handles every command starting with 'TajiMiseClient ' prefix
 */
TajiMiseClient.on('messageCreate', async (msg): Promise<any> => {
    /**
     * filter out bots & no prefix & non guild channels
     */
    if (!msg.content.toLowerCase().startsWith(TajiMiseClient.prefix) || msg.author.bot || !msg.guild) return;

    /**
     * extract commandName: String and args: String[]
     * if !commandName -> just prefix, return;
     */
    const noPrefixMsg = msg.content.slice(TajiMiseClient.prefix.length).trim();
     ...
     */
    const command: BaseCommand = TajiMiseClient.commands.get(commandName);
    if (!command) {
        const embed = new EmbedBuilder()
            .setColor(TajiMiseClient.color)
            .setTitle(
                `No command named "${
                    commandName.length < 10 ? commandName : commandName.slice(0, 10).concat('...')
                }" found`
            )
            .setDescription('Use `taji help` for more information about my commands.')
            ...
            if (!command.args[i].optional && !args.length) {
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(
                        `
**Command usage:**
\`taji ${command.name} ${command.commandUsage}\`
